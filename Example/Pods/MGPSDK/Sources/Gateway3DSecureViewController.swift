...
public enum Gateway3DSecureResult {
    case completed(gatewayResult: OnepayGetwayResult)
...
public class Gateway3DSecureViewController: UIViewController, WKNavigationDelegate {
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        if let url = navigationAction.request.url, let comp = URLComponents(url: url, resolvingAgainstBaseURL: false), comp.scheme == gatewayScheme, comp.host == gatewayHost {
          ...
          do {
            if let JSONString = String(data: gatewayData, encoding: String.Encoding.utf8) {
              print(JSONString)
              let newStr = JSONString.replacingOccurrences(of: "'", with: "\"")
              print(newStr)
              let new_data  = newStr.data(using: .utf8)!
              let gatewayResult = try JSONDecoder().decode(OnepayGetwayResult.self, from: new_data)
              // let gatewayResult = try JSONDecoder().decode(GatewayMap.self, from: gatewayData)
              if gatewayResult.status == 1 {
...
public struct OnepayGetwayResult: Decodable {
